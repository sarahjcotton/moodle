...
class application_cache extends cache implements loader_with_locking_interface {
     ...
     */
    #[\core\attribute\deprecated('application_cache::get_lock()', since: '5.3', mdl: 'MDL-87204')]
     ...
     *      For non-native lock backends, timeout values greater than 0 are advisory only.
    ...
    public function get_lock(string $key, ?int $timeout = null): bool {
        ...
        try {
            ...
            if ($this->nativelocking) {
                ...
                // The non-native lock API is block/non-block and does not support precise timeouts.
                // Preserve current one-shot behaviour and make timeout semantics explicit.
                if ($timeout !== null && $timeout > 0) {
                    debugging(
                        'Non-native cache locks do not support precise timeout values; timeout is advisory only.',
                        DEBUG_DEVELOPER,
                    );
                }
                $lock = $this->cachelockinstance->lock($hashedkey, $this->get_identifier(), false);
            }
