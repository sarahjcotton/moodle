...
final class store_test extends \cachestore_tests {
    ...
    /**
     * Checks that a custom timeout applies only to one call and does not alter default lockwait.
     */
    public function test_lock_custom_timeout_does_not_persist(): void {
        $store = $this->create_cachestore_redis(['lockwait' => 2, 'locktimeout' => 4]);

        // User 123 acquires lock.
        $this->assertTrue($store->get_lock('lock', '123'));

        // User 456 uses non-blocking mode and should fail immediately.
        $before = microtime(true);
        $this->assertFalse($store->get_lock('lock', '456', 0));
        $after = microtime(true);
        $this->assertLessThan(1, $after - $before);

        // User 456 now uses default mode and should still wait for ~2 seconds.
        $before = microtime(true);
        $this->assertFalse($store->get_lock('lock', '456'));
        $after = microtime(true);
        $this->assertEqualsWithDelta(2, $after - $before, 1);
