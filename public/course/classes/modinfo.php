...
class modinfo {
     ...
     *
     * @param stdClass $coursemodinfo object from DB table course
    ...
    public static function compile_course_modinfo(stdClass $coursemodinfodata, int $courseid): stdClass {
        ...
        if ($cachedmods) {
            foreach ($cachedmods as $mod) {
                if ($mod) {
                    $cmid = $mod->data->cm;
                    $sectionid = $mod->data->sectionid;

                    $coursemodinfo->modinfo[$cmid] = $mod->data;

                    if (isset($coursemodinfo->sectioncache[$sectionid])) {
                        $coursemodinfo->sectioncache[$sectionid]->modules[$cmid] = $mod->data;
                    }
                }
            }
        }
        ...
        if (!empty($mods)) {
            do {
                foreach ($mods as $mod) {
                    ...
                    if (!$module) {
                        $module = self::build_module_cache($mod, $course);
                    }
                    if ($module) {
                        $modhits[$module->cm] = $module->cacherev;
                        $coursemodinfo->modinfo[$module->cm] = $module->cacherev;
                    }
                }
            } while (count($modhits) != count($mods));
        }
     ...
     * @return stdClass The module that has been cached
     */
    protected static function build_module_cache(stdClass $mod, stdClass $course): ?stdClass {
        global $DB;
        ...
        $lock = $cache->get_lock($cachekey, 0);
        if (!$lock) {
            return null;
        }
